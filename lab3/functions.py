...
def graph1():
    ...
    plt.hist(list_data, color='aqua', edgecolor='black', bins=25)
    plt.title('Гистограмма времени задач в открытом состоянии')
    plt.xlabel('Время решения (дни)')
    plt.xticks(np.arange(0, int(max_el), int(max_el / 20)))
    plt.ylabel('Количество открытых задач')
    plt.tight_layout()
    plt.show()
    ...
    plt.hist(first_patr, color='aqua', edgecolor='black', bins=25)
    plt.title('Гистограмма времени задач в открытом состоянии')
    plt.xlabel('Время решения (дни)')
    plt.xticks(np.arange(0, int(max_el), int(max_el / 20)))
    plt.ylabel('Количество открытых задач')
...
def graph2():
    ...
    ###### Open
    plt.hist(list_open, color='aqua', edgecolor='black', bins=30)
    ...
    plt.hist(first_patr, color='aqua', edgecolor='black', bins=80)
    ...
    ###### Resolved
    plt.hist(list_resolved_day, color='springgreen', edgecolor='black', bins=30)
    ...
    plt.hist(first_patr, color='springgreen', edgecolor='black', bins=80)
    ...
    plt.hist(second_patr, color='springgreen', edgecolor='black', bins=80)
...
def graph3():
    ...
    plt.plot(list_open_by_day, linewidth=1.5, color='red', label='Открыто')
    ...
    plt.plot(list_close_by_day, linewidth=1.5, color='green', label='Закрыто')

    summary_list_open = summ_elements(list_open_by_day)
    summary_list_close = summ_elements(list_close_by_day)

    plt.plot(summary_list_open, linewidth=1.5, color='orange', label='Открыто(накопительно)')
    plt.plot(summary_list_close, linewidth=1.5, color='purple', label='Закрыто(накопительно)')
...
def graph5(username):
    ...
    payload = {'jql': f'project=KAFKA AND status=Closed AND assignee={username} AND NOT timeestimate = null', 'maxResults': '1000',
               'expand': 'changelog',
               'fields': 'timeestimate'}
    ...
    for elem in data['issues']:
        times_list.append(elem['fields']['timeestimate'] / 3600)
...
def graph6():
    ...
    plt.title(f'График количества задач по степени серьезности')
